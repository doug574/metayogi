 ...
 */
class SelectWidgetElement extends SelectElement implements WidgetInterface
{
    /**
    * {@inheritdoc}
    */
    public function build($properties)
    {
        ...
        $gadgets = array_keys($this->registry->get('widgets'));
        sort($gadgets);
        foreach ($gadgets as $gadgetName) {
            $key = $this->registry->get("widgets.$gadgetName.namespace") . $gadgetName;
